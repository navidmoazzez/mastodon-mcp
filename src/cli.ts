...
function kindOf(schema: ZodTypeAny): { kind: FlagKind; choices?: string[]; repeatable: boolean } {
  ...
  switch (typeName) {
    ...
    case "ZodArray": {
      ...
      // Treating it as JSON meant `--visibility unlisted` was rejected and you
      // had to write `--visibility '"unlisted"'` instead.
      const scalar =
...
export function parseArgs(argv: string[], flags: Flag[]): Record<string, unknown> {
  ...
  // One bare argument fills the first required flag, so `search "cats"` works
  // the way anyone would expect it to before reading any help.
...
export function exitCodeFor(error: unknown): number {
  ...
  // A refused write is the caller's mistake to fix by re-running with
  // --confirm, not a server fault, so it lands on usage rather than api.
 ...
 *
 * The package ships two binaries onto the same file. Printing `mastodon-mcp` at
 * someone who typed `mastodon-cli` hands them a command that works but is not
 * the one they have in their fingers.
 */
function binName(): string {
  const name = (process.argv[1] ?? "").split("/").pop() ?? "";
  return name.startsWith("mastodon-cli") ? "mastodon-cli" : "mastodon-mcp";
}
...
export async function runCli(argv: string[]): Promise<number> {
  ...
  if (!spec) {
    ...
    emitError(
      new Error(
        hidden
          ? `${name} is unavailable: this server is running with MASTODON_READ_ONLY=1.`
